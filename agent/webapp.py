...
async def jira_webhook(
    ...
    Jira Cloud sends a shared secret as a query parameter (?token=...).
    Configure the webhook URL in Jira as:
        https://<your-ngrok-url>/webhooks/jira?token=<JIRA_WEBHOOK_SECRET>

    Supported events:
    - jira:issue_created — starts a new agent run when the issue has the 'open-swe' label
    - jira:issue_updated — re-dispatches when the open-swe label is added
    - comment_created    — dispatches a follow-up run when a comment mentions @open-swe
    """
    logger.info("Received Jira webhook")

    # Jira sends the secret as a query param
    from .utils.jira import verify_jira_webhook_secret

    token_param = request.query_params.get("token", token)
    if not verify_jira_webhook_secret(token_param):
    ...
    if webhook_event in ("jira:issue_created", "jira:issue_updated"):
        result = await process_jira_issue_event(payload)
    elif webhook_event == "comment_created":
        result = await process_jira_comment_event(payload)
    else:
        logger.debug("Ignoring Jira webhook event: %s", webhook_event)
